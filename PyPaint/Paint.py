...
class Paint(Frame):
    ...
    def set_brush_fig(self, new_figure):
    ...
    def set_ui(self):
        ...
        if self.draw_figure is "oval":
            self.canv.bind("<B1-Motion>", self.draw_oval)
        elif self.draw_figure is "req":
            self.canv.bind("<B1-Motion>", self.draw_req)
        ...
        ten_btn = Button(self, text="Req brush", width=10, command=lambda: self.set_brush_fig("req"))
        ten_btn.grid(row=1, column=5)

        twenty_btn = Button(self, text="Oval brush", width=10, command=lambda: self.set_brush_fig("oval"))
